...
public sealed class WinFormsFileStorageBoundarySuite : KeyValueStoreSpec, IPulseLifetime
{
    private readonly string _rootDirectory = Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),
        "Circuids.Pulse",
        "WinFormsConformanceHost");

    protected override string KeyPrefix => "circuids.pulse.sample.winforms.files";

    protected override IConformanceKeyValueStore CreateStore() => new FileSystemKeyValueStore(_rootDirectory);
    ...
    [PulseMatrix(DisplayName = "User-data file batch round-trip", TimeoutMs = 2000)]
    [PulseRow(1)]
    [PulseRow(5)]
    [PulseRow(20)]
    public async Task User_data_file_batch_round_trips_under_budget(int count, CancellationToken ct)
    {
        ...
        try
        {
            foreach (var key in keys)
            {
                ...
                PulseAssert.Equal(key, await store.GetAsync(key, ct), "User-data files must return the value just written.");
            }
        }
        finally
        {
