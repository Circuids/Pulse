...
/// <summary>
/// Demonstrates the per-test cooperative timeout and per-suite lifetime hooks. The suite
/// allocates a shared resource in <see cref="InitializeAsync"/>, exercises it with timeout
/// enforcement on a slow path, and releases it in <see cref="DisposeAsync"/>.
/// </summary>
public sealed class LifetimeAndTimeoutSuite : IPulseLifetime, IAsyncDisposable
{
    ...
    {
        // The token is forwarded by Pulse; if we exceed TimeoutMs, the linked CTS fires and
        // the test is reported as a timeout failure. This call completes well under the budget.
